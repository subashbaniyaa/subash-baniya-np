...
summary: 'Short visual guide on what a Promise is in JavaScript, and how to avoid call back hell.'
---
...
doSomeFetch()
  ...
  .then((result) => doAnotherFetch(result))
...
const outcome = await doAnotherFetch(data, result);
...
try {
  outcome = await doAnotherFetch(data, result);
} catch (e) {
  handleError(e);
}
...
const outcome = await doAnotherFetch(data, result).catch(handleError);
