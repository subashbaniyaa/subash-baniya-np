...
export default function NotPlaying() {
  const { isPlaying, currentSong } = useSpotifyPlayer();
  ...
  return (
    <div className="flex items-center justify-center space-x-2 text-sm sm:justify-start sm:text-base">
      ...
      <div className={`inline-flex space-x-1 ${poppins.className}`}>
        {isPlaying && currentSong ? (
          <>
            ...
            <p className="text-gray-500 dark:text-gray-300">{currentSong.artist}</p>
          </>
        ) : (
          <>
            <p className="font-normal text-gray-800 dark:text-gray-200">
